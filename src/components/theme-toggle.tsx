...
const EVENT = "cockpit-theme-change";
...
export function ThemeToggle() {
  ...
  const flip = () => {
    ...
    try { localStorage.setItem("cockpit-theme", next); } catch {}
    window.dispatchEvent(new Event(EVENT));
  };

  return (
    <button type="button" onClick={flip} className="btn btn-ghost" aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`} title="Theme">
      {theme === "dark" ? (
